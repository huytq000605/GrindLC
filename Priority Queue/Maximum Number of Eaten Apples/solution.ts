function eatenApples(apples: number[], days: number[]): number {
    ...
    while(minHeap.length || day < days.length) {
        while(minHeap.length && day >= minHeap.peek()[1]) {
            minHeap.pop()
        }
        if(day < days.length) {
            if(apples[day] > 0)
                minHeap.push([apples[day], days[day] + day])
        }
        
        if(minHeap.length) {
            let [remainingApples, rottenDay] = minHeap.pop()
            remainingApples--
            result++
            if(remainingApples > 0) {
                minHeap.push([remainingApples, rottenDay])
            }
        }
        day++
    }
