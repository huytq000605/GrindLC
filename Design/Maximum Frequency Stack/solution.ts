class FreqStack {
	...
	pushCount
	constructor() {
			this.maxHeap = new MaxHeap<any>((a,b) => {
					if(a[1] < b[1]) return -1
					if(a[1] === b[1]) {
							if(a[2] > b[2]) return 1
							return -1
	...
	push(val: number): void {
			if(!this.map.has(val)) this.map.set(val, 0)
			this.map.set(val, this.map.get(val) + 1)
			this.maxHeap.push([val, this.map.get(val), this.pushCount++])
	}

	pop(): number {
			const [val, freq] = this.maxHeap.pop()
			this.map.set(val, freq - 1)
			return val
	}
}
